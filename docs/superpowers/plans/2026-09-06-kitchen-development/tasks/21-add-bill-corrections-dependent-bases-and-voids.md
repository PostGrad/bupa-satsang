# Task 21: Add bill corrections, dependent bases and voids
...
**Dependencies:** [20](./20-create-bills-through-one-replay-safe-transaction.md)
...
**Read:** [Binding decisions](../decisions.md); [Contracts](../contracts.md) and [Test fixtures](../test-fixtures.md), sections **C3/C4; F2/F3; bill permission defaults**. Read [Data/API map](../data-api.md) when this card changes persistence or an endpoint.

**Files to inspect/change:**

- `apps/api/src/kitchen/bills/{bill.service,bill.repository}.ts`
- `apps/api/src/sync/mutation.service.ts`
- `apps/api/test/mutation-update.integration.test.ts`
...
**Produces:** bill.update/bill.void handling; same-entity predecessor resolution; history and immutable owner/event.
...
await withTestDb(async db=>{
 ...
 await mutations.apply({userId:ids.shopperA},m);
 const update:BillMutation={...structuredClone(m),mutationId:crypto.randomUUID(),
  kind:'bill.update',base:{kind:'version',value:1}};
 expect((await mutations.apply({userId:ids.shopperA},update)).status).toBe('accepted');
 const competing={...update,mutationId:crypto.randomUUID()};
 expect(await mutations.apply({userId:ids.kitchenHead},competing)).toMatchObject({
  status:'conflict',current:{version:2}
 });
});
```

**Required cases:** Both shopper-own and head/cashier-any corrections; Ravisabha head read-only; stale allocation edits conflict; dependent update uses predecessor version; changed purchaser rejected; reducing amount below allocations rejected; void removes current total once.

- [ ] **Implementation action 1:** Extend real PostgreSQL cases beyond the schema example: create m, submit two independent version1 updates and assert one accepted/one conflict with current bill.
- [ ] **Implementation action 2:** Resolve mutation base only from an accepted same-actor/event/entity/family receipt; compare its version to current state.
- [ ] **Implementation action 3:** Replace all lines/links/allocations transactionally.
- [ ] **Implementation action 4:** Void keeps rows and attributed reason and rejects subsequent editing.
- [ ] **Implementation action 5:** Replay must not reveal current/old records after read permission loss.
...
```sh
pnpm test:api apps/api/test/mutation-update.integration.test.ts
pnpm typecheck
```
